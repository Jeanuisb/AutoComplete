...
public class AutocorrectDemo {
    ...
    public static void main(String[] args)
    {
        ...
        // Uncomment this when you start writing the autocorrector (part D):
        Autocorrector autocorrector = new Autocorrector(wordFreqs);
        //testAutocorrect(autocorrector);

        //Uncomment the two lines below when you're sure the autocorrector is working, and you want to run a GUI demo.
        // To run the GUI demo, comment out the `testAutocorrect()` line above.
        AutocorrectFrame frame = new AutocorrectFrame(autocorrector);
        frame.setVisible(true);
    }

    /**
     * Don't modify this function; I will use it to test your code.
     */
    ...
    private static HashtableMap<String, Integer> processFile(String filename, int size)
    {
        ...
        while (scan.hasNextLine()) {
            ...
            for ( int i = 0; i < words.length; i++)
            {
                ...
                if (wordFreqs.containsKey(words[i]))// if wordFreqs does already has the word:
                {
                    wordFreqs.put(words[i],wordFreqs.get(words[i])+1); //increase the frequency by 1
                }
                else // if the word doesn't exist, create values for it
                {
